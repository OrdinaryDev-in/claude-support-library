...
-- ─── is_anonymous() helper ──────────────────────────────────────────────
-- Mirrors is_admin(uid) (20260815025000_init.sql) in spirit and grant
-- shape: a small, reusable predicate every write policy below can call
-- instead of repeating the raw JWT-claim lookup. GoTrue sets
-- `is_anonymous: true` in the JWT for an anonymous session and
-- omits/falses it otherwise, so the coalesce covers both "false" and
-- "claim absent" as "not anonymous". An anonymous sign-in's JWT carries
-- role: authenticated at the Postgres level (RLS/PostgREST route it as
-- `authenticated`, never as the separate `anon` Postgres role, which
-- means "no JWT at all") — so, like is_admin, this only needs to be
-- callable by `authenticated`; `anon` is revoked explicitly the same way
-- 20260815031028_revoke_is_admin_anon_execute.sql did for is_admin, since
-- this project's default privileges grant EXECUTE to `anon` on every new
