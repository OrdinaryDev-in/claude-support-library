...
import { listReviewQueue, reviewQueueCounts } from "@/lib/data/skills";
import { SkillReviewQueueTable } from "@/components/admin/SkillReviewQueueTable";
import { SKILL_STATUS_META } from "@/lib/constants/review";
import type { SkillStatus } from "@/lib/types/database.types";

export const metadata: Metadata = {
  title: "Skills Review Queue",
  robots: { index: false, follow: false },
};

const TABS: SkillStatus[] = ["pending_review", "approved", "rejected"];

// Mirrors app/(app)/admin/review/page.tsx.
export default async function SkillReviewQueuePage({
  searchParams,
}: {
  ...
  const status: SkillStatus = TABS.includes(rawStatus as SkillStatus) ? (rawStatus as SkillStatus) : "pending_review";
    ...
    <div className="flex-1 w-full mx-auto max-w-[1180px] px-4 sm:px-8 py-8 sm:py-12 pb-24">
      ...
      <div className="flex gap-1 mb-7 border-b border-[var(--border)]">
        {TABS.map((tab) => {
          ...
          const meta = SKILL_STATUS_META[tab];
          return (
            <Link
              key={tab}
              href={`/admin/review/skills?status=${tab}`}
      ...
      {rows.length === 0 ? (
        <div className="text-center py-20 px-5 border border-dashed border-[var(--border)] rounded-lg">
          <div className="font-[family-name:var(--font-mono)] text-[13px] text-[var(--muted)]">
            {status === "pending_review" ? "Nothing waiting for review." : `No ${SKILL_STATUS_META[status].label.toLowerCase()} skills.`}
          </div>
        </div>
      ) : (
        <SkillReviewQueueTable rows={rows} status={status} />
      )}
    </div>
  );
}
