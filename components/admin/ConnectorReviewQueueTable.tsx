...
/** The Connectors counterpart to components/admin/ReviewQueueTable.tsx. */
export function ConnectorReviewQueueTable({ rows, status }: { rows: ReviewQueueRow[]; status: ConnectorStatus }) {
  ...
  function handleApprove(id: string) {
    ...
    startTransition(async () => {
      const result = await approveConnector(id);
      setBusyId(null);
  ...
  function submitReject() {
    ...
    setBusyId(id);
    startTransition(async () => {
      const result = await rejectConnector(id, reason);
      setBusyId(null);
      if (!result.ok) {
        ...
        return (
          ...
          >
            <div className="flex-1 min-w-0">
              ...
              <Link
                href={`/admin/review/connectors/${row.slug}`}
